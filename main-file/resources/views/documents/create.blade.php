...
@endphp
{{ Form::open(['route' => 'documents.store', 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'documentForm']) }}
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select class="form-control multi-select" name="cases" id="cases" placeholder="Select Case" required>
        ...
        <div class="col-md-6 choose-files fw-semibold">
            <label for="profile_pic">
                ...
                <input type="file" class="file" name="file" id="profile_pic" style="width: 0px !important" onchange="document.getElementById('blah').src = window.URL.createObjectURL(this.files[0]); $('#fileName').html($('input[id=profile_pic]').val().split('\\').pop()); image_upload_bar($('input[id=profile_pic]').val().split('.')[1])">
                <span id="fileError" class="text-danger" style="display: none;">{{ __('*Please select a file.')}}</span><br>
                <p>
                    <span class="text-muted m-0">{{ __('Allowed file extension : ') }}{{ $file_validation['mimes'] }}</span><br>
                    <span class="text-muted">({{ __('Max Size  In KB : ') }}{{ $file_validation['max_size'] }})</span><br>
                </p>
