    ...
    @can('create todo')
        <div class="text-sm-end d-flex all-button-box justify-content-sm-end">
            <a href="#" class="btn btn-sm btn-primary mx-1" data-ajax-popup="true" data-size="md" data-title="Add To-Do"
                data-url="{{ route('to-do.create') }}" data-toggle="tooltip" title="{{ __('Create New To-Do') }}"
                data-bs-original-title="{{ __('Create New To-Do') }}" data-bs-placement="top" data-bs-toggle="tooltip">
                                ...
                                <select name="filter" class="form-control select" id="priorities">
                                    @foreach ($priorities as $priority)
                                        <option value="{{ $priority }}"
                                            {{ isset(request()->filter) && request()->filter == $priority ? 'selected' : '' }}>
                                            {{ $priority }}
                                        </option>
                                    @endforeach
                                </select>
                                                ...
                                                @can('edit todo')
                                                    <div class="action-btn  disabled-form-switch">
                                                        <a href="#" data-size="md"
                                                            ...
                                                            data-title="{{ $todo->status == 1 ? __('Mark Completed?') : __('Completed') }}"
                                                            data-bs-toggle="tooltip" data-bs-placement="top" title=""
                                                            data-bs-original-title="{{ $todo->status == 1 ? __('Mark Completed?') : __('Completed') }}">
                                                            @if ($todo->status == 0)
                                                                <input type="checkbox" class="form-check-input "
                                                                    disabled="disabled" name="status" id="{{ $todo->id }}"
...
@push('custom-script')
    <script>
        $(document).ready(function() {
            var case_id = $('#relate_to').val();
            getAdvocte(case_id);
        });
        $(document).on('change', 'select[name=relate_to]', function() {
            ...
            if (case_id) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        $("#advocate_div").html('');
                        $('#advocate_div').append(
                            '<select class="form-control multi-select" name="assign_to[]" id="assign_to" multiple></select>'
                        );
                        ...
                        $.each(result.data, function(key, value) {
                            ...
                            $('#assign_to').append('<option value="' + key + '">' +
                                value + '</option>');
                        });

                        var multipleCancelButton = new Choices('#assign_to', {
