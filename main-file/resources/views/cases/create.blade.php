...
@section('content')
    ...
    {{ Form::open(['route' => 'cases.store', 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'documentForm']) }}
    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12">
                            <div class="form-group">
                                ...
                                {{ Form::text('judge', null, ['class' => 'form-control', 'required' => 'required']) }}
                            </div>
                        </div>
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12">
                            <div class="form-group">
                                ...
                                {{ Form::number('court_room', null, ['class' => 'form-control', 'required' => 'required']) }}
                            </div>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 choose-files fw-semibold ">
                            <label for="case_docs" class="upload__btn">
                                ...
                                <p>
                                    <span class="text-muted m-0 file-info">{{ __('Allowed file extension : ') }}{{ $file_validation['mimes'] }}</span><br>
                ...
                success: function(data) {
                    if (data.status == 1) {
                        ...
                        $('#casetype_div').removeClass('d-none').append(
                            ...
                            '</label><select class="form-control multi-select" name="casetype" id="casetype"> <option value="">{{ __('Please Select') }}</option><option value="Case Number">{{ __('Case Number') }}</option><option value="Diary Number">{{ __('Diary Number') }}</option></select>'
                        );
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        $('#bench_div').empty();
                        $('#bench_div').append(
                            '<label for="bench" class="form-label">Bench</label> <select class="form-control multi-select" name="bench" id="bench"> </select>'
                        );
            ...
            $("#documentForm").submit(function(event) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    if (file.size > maxSize) {
                        $("#fileError").text("*The file size should be less than {{ $file_validation['max_size'] }}KB for each file.").show();
                        return false;
                    }
                    ...
                    if ($.inArray('.'+fileType, allowedTypes) === -1) {
                        $("#fileError").text("*Please upload a valid file type ({{ $file_validation['mimes'] }}) for each file.").show();
                        return false;
                    }
                }
                return true;
