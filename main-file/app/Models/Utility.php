...
class Utility extends Model
{
    ...
    public static function notification()
    {
        ...
        public static function referralcommisonadd($planID)
    {
        // dd(Auth::user()->use_refercode);
        $setting = Utility::settings();
        $existingRecords = referral_transaction::where('user_id', Auth::user()->id)->get();
        if ($existingRecords->isEmpty()  && $setting['referral_status']=='on' && Auth::user()->use_refercode != null) {

            $plan = Plan::find($planID);
            referral_transaction::create([
                'user_id' => Auth::user()->id,
                'plan' => $plan->id,
                'commisson' => $setting['commission_percentage'],
                'commisson_amount' => $plan->price * $setting['commission_percentage'] / 100,
            ]);
        }
    }
