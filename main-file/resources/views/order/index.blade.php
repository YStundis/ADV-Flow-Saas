                                ...
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if (!in_array($order->user_id, $refundDisabled) && $order->refund == 0)
                                                @if (!isset($latestTimestamps[$order->user_id]) || $latestTimestamps[$order->user_id] < $order->created_at)
                                                    @php
                                                        $latestTimestamps[$order->user_id] = $order->created_at;
                                                    @endphp

                                                    <div class="action-btn bg-light-secondary ms-2">
                                                        <a href="{{ route('plan.refund', [$order->user_id, $order->id]) }}"
                                                            class="mx-3 btn btn-sm align-items-center"
                                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                                            title="{{ __('Refund') }}">
                                                            <i class="ti ti-receipt-refund m-1"></i>
                                                        </a>
                                                    </div>
                                                @endif
                                            @else
                                                @php
                                                    $refundDisabled[] = $order->user_id;
                                                @endphp
                                            @endif
